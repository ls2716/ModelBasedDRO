...
def run_single(
    ...
    for delta_mb in delta_mb_arr:
        ...
        optimised_prices_mb.append(robust_price_mb)
    print("Optimised robust prices for MB method for all deltas")
    print("------------------------")
    deltas = np.array([0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1])
    robust_profits_mb = {}
    for delta_mb, optimised_robust_price_mb in zip(delta_mb_arr, optimised_prices_mb):
        r_profits = []
        for delta in deltas:
    ...
    # Plot the results
    plot_path = f"results/shift_experiment_dro_alpha_{alpha}_beta_{beta}.png"
...
if __name__ == "__main__":
    ...
    delta_mb_arr = [0.001, 0.005, 0.02, 0.05, 0.1]
